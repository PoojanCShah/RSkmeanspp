...
k_vals = [2,5,10,15,20]
m_vals = [2,5,10,20,30,40,50,75,100,120,150,200,250,300]
...
for k in k_vals : 
    ...
    with open(f"logs/exp2.{data_name}(k={k}).csv", 'w') as f : 

        f.write(f"m, rskmeanspp cost, afkmc2 cost, rskmeanspp std, afkmc2 std, rskmeanspp time, afkmc2 time\n")
        ...
        for m in m_vals : 
            ...
            f.write(f"{m}, {np.mean(rskmeanspp_costs)}, {np.mean(afkmc2_costs)}, {np.std(rskmeanspp_costs)}, {np.std(afkmc2_costs)}, {np.mean(rskmeanspp_times)}, {np.mean(afkmc2_times)}\n")
